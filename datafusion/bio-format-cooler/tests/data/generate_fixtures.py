...
# Coordinates use standard int64 storage and can exceed UInt32. The second bin
# begins above u32::MAX, catching any narrowing intermediate or output schema.
...
with h5py.File("test_wide_coords.cool", "r+") as h5:
    ...
    h5["bins"].create_dataset(
        "start", data=np.array([0, 5_000_000_000], dtype="int64")
    )
    h5["bins"].create_dataset(
...
print("weights present:", "weight" in cooler.Cooler("test.mcool::/resolutions/1000").bins().columns)
