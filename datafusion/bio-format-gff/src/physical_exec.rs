...
pub struct GffExec {
    ...
    /// Genomic regions for index-based queries (one per partition)
...
/// Temporary wrapper for remote GFF records to implement GffRecordTrait
/// This allows us to use the same filter evaluation logic for both local and remote records
#[derive(Debug)]
struct RemoteGffRecordWrapper {
    chrom: String,
    start: u32,
    end: u32,
    ty: String,
...
impl GffRecordTrait for RemoteGffRecordWrapper {
...
> {
    ...
    let stream = try_stream! {
        ...
        while let Some(result) = records.next().await {
            ...
            // For remote records, we need to create a temporary wrapper that implements GffRecordTrait
            let temp_record = RemoteGffRecordWrapper {
                chrom: record.reference_sequence_name().to_string(),
                start: record.start().get() as u32,
                end: record.end().get() as u32,
                ty: record.ty().to_string(),
                source: record.source().to_string(),
                score: record.score(),
                strand: standardize_strand(record.strand()).to_string(),
                phase: standardize_phase(record.phase()).map(|p| p as u8),
                attributes: String::new(), // Will be populated below if needed
            };
...
/// Get a stream of RecordBatches from an indexed GFF file for a specific region.
///
/// Uses `spawn_blocking` because noodles' indexed reader holds a trait object
/// (`dyn BinningIndex`) which is not `Send`. All I/O is done synchronously inside
/// the blocking task, which builds all batches and then streams them out.
#[allow(clippy::too_many_arguments)]
async fn get_indexed_gff_stream(
    ...
    let batches =
        tokio::task::spawn_blocking(move || -> Result<Vec<RecordBatch>, DataFusionError> {
            ...
            // Remaining records
            if !chroms.is_empty() || (record_num % batch_size != 0 && record_num > 0) {
                let remaining = record_num % batch_size;
                if remaining > 0 {
                    let batch = build_record_batch_optimized(
                        Arc::clone(&schema),
                        &chroms,
            ...
            debug!(
                "Indexed GFF scan: {} records for region {:?}",
                record_num, region
            );
            Ok(all_batches)
        })
        .await
        .map_err(|e| DataFusionError::Execution(format!("Indexed GFF task join error: {}", e)))??;
