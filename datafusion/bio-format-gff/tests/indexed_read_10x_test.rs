//! Integration tests for index-based predicate pushdown with 10x larger GFF data.
//!
...
//!
//! The test GFF uses chr-prefixed chromosome names (chr1, chr2, chrX).
//! Uses multi_chrom_10x.gff3.gz: 448 features across chr1(150), chr2(208), chrX(90).
...
/// Create a session context with the multi_chrom_10x.gff3.gz test file registered.
async fn setup_gff_ctx() -> datafusion::error::Result<SessionContext> {
    let ctx = SessionContext::new();
    let provider = GffTableProvider::new(
        "tests/multi_chrom_10x.gff3.gz".to_string(),
        None, // attr_fields
        None, // thread_num
...
#[tokio::test]
async fn test_gff_single_region_query_10x() -> datafusion::error::Result<()> {
...
#[tokio::test]
async fn test_gff_multi_chromosome_query_10x() -> datafusion::error::Result<()> {
    let ctx = setup_gff_ctx().await?;
...
#[tokio::test]
async fn test_gff_full_scan_total_count_10x() -> datafusion::error::Result<()> {
    let ctx = setup_gff_ctx().await?;
...
#[tokio::test]
async fn test_gff_record_level_filter_10x() -> datafusion::error::Result<()> {
    let ctx = setup_gff_ctx().await?;
...
#[tokio::test]
async fn test_gff_combined_filters_10x() -> datafusion::error::Result<()> {
    let ctx = setup_gff_ctx().await?;
...
#[tokio::test]
async fn test_gff_region_with_positions_10x() -> datafusion::error::Result<()> {
    let ctx = setup_gff_ctx().await?;

    let chr1_total = count_rows(&ctx, "SELECT chrom FROM gff WHERE chrom = 'chr1'").await;

    // Query a subregion of chr1 (0-based coordinates since we use zero_based=true)
    // Original 1-based range: 1000-15000, so 0-based: start >= 999 AND end <= 15000
    let region_count = count_rows(
        &ctx,
        "SELECT chrom FROM gff WHERE chrom = 'chr1' AND start >= 999 AND \"end\" <= 15000",
    )
    .await;
...
#[tokio::test]
async fn test_gff_indexed_correctness_10x() -> datafusion::error::Result<()> {
    let ctx = setup_gff_ctx().await?;
