//! Integration tests for index-based predicate pushdown with 10x larger BAM data.
//!
...
//!
//! Uses multi_chrom_10x.bam: 421 reads across chr1(160), chr2(159), chrX(102).
...
/// Create a session context with the multi_chrom_10x.bam test file registered.
async fn setup_bam_ctx() -> datafusion::error::Result<SessionContext> {
    let ctx = SessionContext::new();
    let provider = BamTableProvider::new(
        "tests/multi_chrom_10x.bam".to_string(),
        None, // thread_num
...
#[tokio::test]
async fn test_bam_single_region_query_10x() -> datafusion::error::Result<()> {
...
#[tokio::test]
async fn test_bam_multi_chromosome_query_10x() -> datafusion::error::Result<()> {
    let ctx = setup_bam_ctx().await?;
...
#[tokio::test]
async fn test_bam_full_scan_total_count_10x() -> datafusion::error::Result<()> {
    let ctx = setup_bam_ctx().await?;
...
#[tokio::test]
async fn test_bam_record_level_filter_10x() -> datafusion::error::Result<()> {
    let ctx = setup_bam_ctx().await?;
...
#[tokio::test]
async fn test_bam_combined_genomic_and_record_filter_10x() -> datafusion::error::Result<()> {
    let ctx = setup_bam_ctx().await?;
...
#[tokio::test]
async fn test_bam_region_with_start_end_10x() -> datafusion::error::Result<()> {
    let ctx = setup_bam_ctx().await?;
...
#[tokio::test]
async fn test_bam_indexed_vs_full_scan_correctness_10x() -> datafusion::error::Result<()> {
    let ctx = setup_bam_ctx().await?;
