...
///
/// The RecordBatch must have columns matching CRAM schema names. Columns are
...
/// * `coordinate_system_zero_based` - If true, coordinates are 0-based (need +1 for CRAM POS)
///
/// # Returns
///
/// A vector of CRAM records that can be written to a file
///
/// # Errors
///
/// Returns an error if required columns are missing or have wrong types
pub fn batch_to_cram_records(
    batch: &RecordBatch,
    header: &sam::Header,
    tag_fields: &[String],
...
/// Builds a single CRAM record from Arrow array values
...
mod tests {
    ...
    fn test_batch_to_bam_records_basic() -> Result<()> {
        ...
        let records = batch_to_cram_records(&batch, &header, &[], true)?;
