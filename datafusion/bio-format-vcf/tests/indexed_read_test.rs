...
async fn test_vcf_single_region_query() -> datafusion::error::Result<()> {
    ...
    assert!(count > 0, "Expected variants on chr 21");
...
async fn test_vcf_full_scan_total_count() -> datafusion::error::Result<()> {
    ...
    assert!(total > 0, "Expected some variants in full scan");
...
async fn test_vcf_record_level_filter() -> datafusion::error::Result<()> {
    ...
    assert!(
        filtered < total,
        "Filtered count ({}) should be < total ({}) since some variants have qual < 30",
        filtered,
        total
    );
