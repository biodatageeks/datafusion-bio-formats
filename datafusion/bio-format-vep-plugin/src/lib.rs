...
impl PluginSourceKind {
    ...
    pub fn select_query(self) -> &'static str {
        match self {
            ...
            Self::DbNSFP => {
                "SELECT chr AS chrom, CAST(\"pos(1-based)\" AS INTEGER) AS pos, ref AS ref, alt AS alt, CAST(sift4g_score AS VARCHAR) AS sift4g_score, CAST(sift4g_pred AS VARCHAR) AS sift4g_pred, CAST(polyphen2_hdiv_score AS VARCHAR) AS polyphen2_hdiv_score, CAST(polyphen2_hvar_score AS VARCHAR) AS polyphen2_hvar_score, CAST(NULL AS VARCHAR) AS lrt_score, CAST(NULL AS VARCHAR) AS lrt_pred, CAST(mutationtaster_score AS VARCHAR) AS mutationtaster_score, CAST(mutationtaster_pred AS VARCHAR) AS mutationtaster_pred, CAST(NULL AS VARCHAR) AS fathmm_score, CAST(NULL AS VARCHAR) AS fathmm_pred, CAST(provean_score AS VARCHAR) AS provean_score, CAST(provean_pred AS VARCHAR) AS provean_pred, CAST(vest4_score AS VARCHAR) AS vest4_score, CAST(metasvm_score AS VARCHAR) AS metasvm_score, CAST(metasvm_pred AS VARCHAR) AS metasvm_pred, CAST(metalr_score AS VARCHAR) AS metalr_score, CAST(metalr_pred AS VARCHAR) AS metalr_pred, CAST(revel_score AS VARCHAR) AS revel_score, CAST(\"gerp++_rs\" AS VARCHAR) AS gerp_rs, CAST(phylop100way_vertebrate AS VARCHAR) AS phylop100way, CAST(NULL AS VARCHAR) AS phylop30way, CAST(phastcons100way_vertebrate AS VARCHAR) AS phastcons100way, CAST(NULL AS VARCHAR) AS phastcons30way, CAST(NULL AS VARCHAR) AS siphy_29way, CAST(cadd_raw AS VARCHAR) AS cadd_raw, CAST(cadd_phred AS VARCHAR) AS cadd_phred, CAST(aapos AS VARCHAR) AS __vepyr_dbnsfp_aapos, CAST(aaref AS VARCHAR) AS __vepyr_dbnsfp_aaref, CAST(aaalt AS VARCHAR) AS __vepyr_dbnsfp_aaalt FROM source"
            }
        }
    }
