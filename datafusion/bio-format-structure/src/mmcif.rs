...
pub fn parse(data: &[u8], options: &StructureOptions) -> Result<Vec<NormalizedEntry>> {
    ...
    for (block_index, b) in doc.blocks()?.iter().enumerate() {
        ...
        for row in 0..n {
            let result = (|| -> Result<Atom> {
                ...
                let coord = |tag: &str| {
                    float(get(tag), tag)?.ok_or_else(|| error(format!("missing {tag}")))
                };
                Ok(Atom {
            ...
            entry.atoms.push(result.map_err(|e| {
                error(format!(
                    "mmCIF block {:?}, atom row {}: {e}",
                    b.name,
                    row + 1
                ))
            })?);
